...
void initialize() {
  pros::Task display_task([&](){
	lcd.draw("(Y): " + config::get_control_scheme(bat.control_type));
	// Add more into this lambda.
	// This is the task for the controller screen, which we use to draw onto the screen.
  });
}
